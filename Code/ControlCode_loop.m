...
for PCA = 0:1 %Change if you want to do PCA (0 = no; 1 = yes)
    for sensors = 0:1 % Change if you want to see sensordata: no sensors = 0, sensors = 1
        for condition = 1:4 %Change between Nothing = 0, Matched = 1 or OneHead = 2, Random = 3, OneData = 4
            if sensors==1 && condition>1
                break
            end
            for analyse = 1 %Change to choose the analysis you want 0 = Nothing ,1 = timedomain, 2 = AEC no ort, 3 = ciPLV, 4 = AEC ort
                for fq_index = 1:5 %Decide the frequency for the frequency measures. 0=time domain, don't set here; 1=[0.5;4];2=[4;8];3=[8;13];4=[13;30];5=[30;48]
                    fq=fq_tot(:,fq_index);
